...
class PDB:
    ...
    def parse(self):
        ...
        f = open(self.file, 'r')
        lines = f.readlines()
        self.terminal = int(lines[-5][22:26])
        f.close()
    ...
    def get_grid(self, atoms):
        ...
        grid = numpy.zeros((i,j,k), dtype = numpy.int)
        cal = 0
        vol = 0
        #print 'checking..'
        for atom in atoms:
            ...
            flag = False
            for pos in lower_position:
                if (pos >= 60) or (pos <= -60):
                    flag = True
            for pos in higher_position:
                if (pos >= 60) or (pos <= -60):
                    flag = True
            if flag == True:
                continue

            for x in range(lower_index[0], higher_index[0]+1):
                for y in range(lower_index[1], higher_index[1]+1):
                    for z in range(lower_index[2], higher_index[2]+1):
                        if(grid[x, y, z] != 0):
                                continue
                        #distance
...
def listTojson(training_data_list, feature, file):
    tmp = {}
    tmp['feature'] = feature
    print file
    filename = file[49:]
    rootname = file[:49] + "/list.dat"
    gdtts = -1
    f = open(rootname, 'r')
    for line in f.readlines()[1:]:
        data = line.split("\t")
        if data[0] == filename:
            ...
            break
    tmp['label'] = gdtts
    training_data_list.append(tmp)


if __name__ == '__main__':
    import sys
    training_data_list = []
    targetPath = "/Users/yuelyu/Documents/project/CASP_SCWRL"
    targetList=list(os.path.join(targetPath,name) for name in os.listdir(targetPath))
    randomTarget=list(random.sample(targetList,1)) #change this number to get more targets.
    num_of_loop = 0
    
    for decoysPath in randomTarget:
        randomDecoy = getDecoysList(decoysPath)

        for file in randomDecoy:
            ...
            listTojson(training_data_list, res, file)
            num_of_loop += 1
            print "number of loop : " + str(num_of_loop)

    print "number of training records: " + str(len(training_data_list))
    with open("/Users/yuelyu/Documents/2018fall/simulation/project/trainingdata/trainingdata_1.json","w") as f:
        json.dump(training_data_list,f)
        print "json writing finished"
